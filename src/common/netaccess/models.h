...
template<typename T>
concept ValidAttributeType = std::same_as<T, int> || 
                             std::same_as<T, std::string> || 
                             std::same_as<T, bool> || 
                             std::same_as<T, fs::perms>;

struct Subject {
    std::string id;
    std::map<std::string, AttributeValue, std::less<>> attributes;

    template<ValidAttributeType T>
    [[nodiscard]] T get_attribute(std::string_view key, T default_value) const {
